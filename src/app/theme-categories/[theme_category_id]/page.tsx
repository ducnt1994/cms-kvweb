...
import {addThemeCategory, getDetailThemeCategory, updateThemeCategory} from "@/api/theme-category";
import {ICreateThemeDto} from "@/constants/theme-categories";
import {useParams, useRouter} from "next/navigation";

export default function DetailThemeCategory() {
  const router = useRouter()
  const searchParams = useParams()
  const themeCategoryId = searchParams.theme_category_id as string;
  ...
  const submitData = async (data : ICreateThemeDto) => {
    ...
    try {
      const res = await updateThemeCategory(themeCategoryId, data)
      if (res) {
  ...
  const loadDetailCategory = async () => {
    const res = await getDetailThemeCategory(themeCategoryId)
    if (res){
      setValue('name', res.name)
      setValue('child_categories', res.child_categories)
    ...
    >
      ...
      <Paper elevation={3} sx={{ p: 2, mt: 2 }}>
        <form onSubmit={(e) => {
          ...
          <Button type={'submit'} variant={'contained'} sx={{mt: 3}}>Cập nhật</Button>
        </form>
      </Paper>
    </Box>
  </>
